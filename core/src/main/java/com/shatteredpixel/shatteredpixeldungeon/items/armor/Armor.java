...
public class Armor extends EquipableItem {
	...
	protected int dmgRedBonus=0; //damge reduction bonus
	protected int bonusSTRReq=0; // bonus STR required
	...
	public int DRMax(int lvl){
		int max = tier * (2 + lvl) + augment.defenseFactor(lvl)+dmgRedBonus*(1+level/5);
		if (lvl > max){
			return ((lvl - max)+1)/2;
		} else {
			return max;
	...
	public int DRMin(int lvl){
		int max = DRMax(lvl)+ dmgRedBonus*(1+level/5);
		if (lvl >= max){
			return (lvl - max);
		} else {
			return lvl;
	...
	public int STRReq(int lvl){
		...
		return (8 + Math.round(tier * 2)) - (int)(Math.sqrt(8 * lvl + 1) - 1)/2+bonusSTRReq;
	}
	...
	public int price() {
		...
		int price = 20 * tier+dmgRedBonus*15;
		if (hasGoodGlyph()) {
			price *= 1.5;
		}

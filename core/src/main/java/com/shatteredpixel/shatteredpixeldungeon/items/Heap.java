	...
	public void burn() {
		...
		for (Item item : items.toArray( new Item[0] )) {
			if (item instanceof Scroll
				...
				burnt = true;
			}  else if (item instanceof DeadRat) {
				replace( item, TailedCutlet.cook( (DeadRat) item ) );
				burnt = true;
			}
			else if (item instanceof BurdockRoot) {
				replace( item, FriedBurdockRoot.cook( (BurdockRoot) item ) );
				burnt = true;
			}
			else if (item instanceof WildCarrot) {
				replace( item, FriedCarrot.cook( (WildCarrot) item ) );
				burnt = true;
			}
			else if (item instanceof ChicoryRoot) {
				replace( item, FriedChicoryRoot.cook( (ChicoryRoot) item ) );
				burnt = true;
			}

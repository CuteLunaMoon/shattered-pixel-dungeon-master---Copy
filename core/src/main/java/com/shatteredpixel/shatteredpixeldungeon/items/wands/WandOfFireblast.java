...
public class WandOfFireblast extends DamageWand {
	...
	public int min(int lvl){
		return (int)Math.round((1+lvl) * Math.pow(1.5f, chargesPerCast()-1))+Dungeon.hero.ARC;
	}

	//1x/1.5x/2.25x damage
	public int max(int lvl){
		return (int)Math.round((5+3*lvl) * Math.pow(1.5f, chargesPerCast()-1))+Dungeon.hero.ARC;
	}
